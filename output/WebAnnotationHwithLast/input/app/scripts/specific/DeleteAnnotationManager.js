...
class DeleteAnnotationManager {
  ...
  init (callback) {
    ...
    this.events.annotationsDeleted = {element: document, event: Events.deletedAllAnnotations, handler: this.createAnnotationsDeletedEventHandler()}
    this.events.annotationsDeleted.element.addEventListener(this.events.annotationsDeleted.event, this.events.annotationsDeleted.handler, false)
    console.log('Handler creado: ' + this.events.annotationsDeleted)
  ...
  createAnnotationsDeletedEventHandler () {
	  return (event) => {
		  console.log(event.detail.allAnnotations)
		  this.deleteClassificationsFromHypersheet(event.detail.allAnnotations, (err) => {
			  if (err) {
				  console.error(err)
			  } else {
				  console.debug('Correctly updated google sheet with deleted annotation')
			  }
		  })
	  }
  }
