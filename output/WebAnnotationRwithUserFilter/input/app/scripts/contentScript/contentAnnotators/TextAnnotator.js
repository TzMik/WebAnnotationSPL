  ...
  */
  ...
  commentAnnotationHandler (annotation) {
	...
	Alerts.multipleInputAlert({
	  ...
	  callback: (err, result) => {
	    updateAnnotation({comment: newComment/**/}/**/, level/**/)
	    if (isSidebarOpened) {
	      this.openSidebar()
	    }
	  }
  ...
  mouseUpOnDocumentHandlerConstructor () {
    return (event) => {
      ...
      if (document.getSelection().toString().length !== 0) {
        if ($(event.target).parents('#abwaSidebarWrapper').toArray().length === 0 /**/  && $(event.target).parents('.swal2-container').toArray().length === 0 /* */){
          this.openSidebar()
        }
      } else {
